...
private:
	...
	// the struct to hold camera Intrinsics param
	struct CameraIns {
		cv::Size image_size_;
	...
	CameraIns camera_ins_;

	tf::StampedTransform transform_;
	bool camera_lidar_tf_ok_ = false;
...
public:
	...
	mapOptimization() {
		...
		debugPublisher = nh.advertise<sensor_msgs::PointCloud2>("lio_sam/mapping/rgb_cloud", 1);
		...
		hasCamInfo = false;
		subCamInfo = nh.subscribe<sensor_msgs::CameraInfo>(cameraInfoTopic, 10, &mapOptimization::camInfoHandler, this,
		                                                   ros::TransportHints().tcpNoDelay());
	...
	void laserCloudInfoHandler(const lio_sam::cloud_infoConstPtr &msgIn) {
		...
		if (useImageData && hasCamInfo) {
			// use shared ptr to received images
			currentImage_ = msgIn->image_data;
		}
	...
	void camInfoHandler(const sensor_msgs::CameraInfo::ConstPtr &camInfoMsg) {
		if (!hasCamInfo) {
			...
			camera_ins_.image_size_.height = camInfoMsg->height;
			camera_ins_.image_size_.width = camInfoMsg->width;

			camera_ins_.camera_instrinsics_ = cv::Mat(3, 3, CV_64F);
			for (int row = 0; row < 3; row++) {
				for (int col = 0; col < 3; col++) {
					camera_ins_.camera_instrinsics_.at<double>(row, col) = camInfoMsg->K[row * 3 + col];
				}
			}

			camera_ins_.distortion_coefficients_ = cv::Mat(1, 5, CV_64F);
			for (int col = 0; col < 5; col++) {
				camera_ins_.distortion_coefficients_.at<double>(col) = camInfoMsg->D[col];
			}

			camera_ins_.fx_ = static_cast<float>(camInfoMsg->P[0]);
			camera_ins_.fy_ = static_cast<float>(camInfoMsg->P[5]);
			camera_ins_.cx_ = static_cast<float>(camInfoMsg->P[2]);
			camera_ins_.cy_ = static_cast<float>(camInfoMsg->P[6]);

			// do not need sub to camera info anymore
			subCamInfo.shutdown();
			hasCamInfo = true;
...
#pragma omp parallel for num_threads(numberOfCores)
	...
	transformRGBPointCloud(pcl::PointCloud<pcl::PointXYZRGB>::Ptr cloudIn, PointTypePose *transformIn) {
...
#pragma omp parallel for num_threads(numberOfCores)
	...
	pcl::PointXYZ transformPoint(const PointType &in_point, const tf::Transform &in_transform) {
		tf::Vector3 tf_point(in_point.x, in_point.y, in_point.z);
		tf::Vector3 tf_point_t = in_transform * tf_point;
		return pcl::PointXYZ(tf_point_t.x(), tf_point_t.y(), tf_point_t.z());
	}

	pcl::PointCloud<pcl::PointXYZRGB>::Ptr projectRGBToCloud(pcl::PointCloud<PointType>::Ptr input_cloud,
	                                                         sensor_msgs::Image input_img,
	                                                         tf::Transform cam_to_lidar) {
		...
		std::vector<pcl::PointXYZ> cam_cloud(input_cloud->points.size());
		for (size_t i = 0; i < input_cloud->points.size(); i++) {
			cam_cloud[i] = transformPoint(input_cloud->points[i], cam_to_lidar);
			int u = int(cam_cloud[i].x * camera_ins_.fx_ / cam_cloud[i].z + camera_ins_.cx_);
			int v = int(cam_cloud[i].y * camera_ins_.fy_ / cam_cloud[i].z + camera_ins_.cy_);
			if ((u >= 0) && (u < camera_ins_.image_size_.width)
			    && (v >= 0) && (v < camera_ins_.image_size_.height)
			    && cam_cloud[i].z > 0) {
				projection_map.insert(std::pair<cv::Point, PointType>(cv::Point(u, v), input_cloud->points[i]));
			}
		}

		out_cloud->points.clear();

		for (int row = 0; row < camera_ins_.image_size_.height; row++) {
			for (int col = 0; col < camera_ins_.image_size_.width; col++) {
				std::unordered_map<cv::Point, PointType>::const_iterator iterator_3d_2d;
				PointType corresponding_3d_point;
				...
				if (iterator_3d_2d != projection_map.end()) {
					...
					cv::Vec3b rgb_pixel = cv_mat.at<cv::Vec3b>(row, col);
	...
	void visualizeGlobalMapThread() {
		...
		////////////// image rgb project to point cloud ///////////////
		if(useImageData){
...
#pragma omp parallel for num_threads(numberOfCores)
	...
	void saveKeyFramesAndFactor() {
		...
		imageList.push_back(currentImage_);
