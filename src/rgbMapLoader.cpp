...
sensor_msgs::PointCloud2 MapLoader::TransformMap(sensor_msgs::PointCloud2 & in){
    ...
    pcl::PointCloud<pcl::PointXYZRGB>::Ptr transformed_pc_ptr(new pcl::PointCloud<pcl::PointXYZRGB>);
...
void MapLoader::SaveMap(const pcl::PointCloud<pcl::PointXYZRGB>::Ptr map_pc_ptr){
    pcl::io::savePCDFileBinary("/tmp/transformed_map.pcd", *map_pc_ptr);
}
